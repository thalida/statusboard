...
@pytest.mark.django_db
def test_component_admin_fieldsets_order_featured_and_archive():
    # Featured moved to the base section. Archive fields moved to their
    # own section. Verify the fieldsets layout against regressions.
    from catalog.models import ServiceComponent

    model_admin = admin.site._registry[ServiceComponent]
    fieldsets = model_admin.get_fieldsets(None)

    # Four sections: base, Position, Archive, audit.
    assert len(fieldsets) == 4, f"Expected 4 fieldsets, got {len(fieldsets)}"

    # First section (base): has is_featured, not archive fields.
    base_section = fieldsets[0]
    assert base_section[0] is None, "First section should have no title"
    base_fields = base_section[1]["fields"]
    assert "is_featured" in base_fields, "is_featured must be in base section"
    assert "is_archived" not in base_fields, "is_archived must not be in base section"
    assert "archived_at" not in base_fields, "archived_at must not be in base section"

    # Second section: Position (unchanged).
    position_section = fieldsets[1]
    assert "Position" in str(position_section[0]), "Second section must be Position"
    assert "parent" in position_section[1]["fields"], (
        "Position section must have parent"
    )

    # Third section: Archive with archive fields.
    archive_section = fieldsets[2]
    assert "Archive" in str(archive_section[0]), "Third section must be Archive"
    archive_fields = archive_section[1]["fields"]
    assert "is_archived" in archive_fields, "is_archived must be in Archive section"
    assert "archived_at" in archive_fields, "archived_at must be in Archive section"

    # Fourth section: audit (checked by presence).
    assert fieldsets[3] is not None, "Fourth section (audit) must exist"
